...
# ── HELPERS ──
def ts_now():  return datetime.now().strftime("%H:%M:%S")
...
def add_threat(severity, category, title, detail,
    ...
    threading.Thread(target=save_json,
                     ...
                     daemon=True).start()
    # Auto-create or update incident
...
def auto_incident(alert):
    ...
    ip = alert.get("ip","")
    with lock:
        # Find existing open incident for this IP
        existing = None
...
def run_hunt(query, hunt_type="keyword"):
    ...
    for c in conns:
        score = 0
        matches = []
        if q in (c.get("rip","") or "").lower():    score+=3; matches.append(f"Remote IP: {c['rip']}")
        if q in (c.get("domain","") or "").lower():  score+=3; matches.append(f"Domain: {c['domain']}")
        if q in (c.get("process","") or "").lower(): score+=2; matches.append(f"Process: {c['process']}")
        if q in str(c.get("rport","")).lower():      score+=2; matches.append(f"Port: {c['rport']}")
        if score:
            results.append({
                "source":"Connection","severity": "high" if c.get("malicious") else "medium" if c.get("suspicious") else "low",
                "match": " | ".join(matches), "detail": f"{c['process']} → {c['raddr']} [{c['status']}]",
    ...
    for p in procs:
        if q in (p.get("name","") or "").lower() or q in (p.get("exe","") or "").lower():
            results.append({
                "source":"Process","severity":"critical" if p.get("suspicious") else "medium",
                "match": f"Process name: {p['name']}",
                "detail": f"PID {p['pid']} | CPU {p['cpu']}% | {p['connections']} connections",
    ...
    for t in threats[:500]:
        if (q in (t.get("title","") or "").lower() or
            ...
            results.append({
                "source":"Threat Log","severity": t.get("severity","medium"),
                "match": f"Threat: {t['title']}",
                "detail": t.get("detail",""), "ip": t.get("ip",""),
    ...
    for p in ports:
        if q in str(p.get("port","")) or q in (p.get("process","") or "").lower():
            results.append({
                "source":"Open Port","severity":"high" if p.get("suspicious") else "low",
                "match": f"Port {p['port']} — {p['process']}",
...
def lookup_cve_for_port(port, service=""):
    """Query NVD for CVEs related to a port/service."""
    cache_key = f"{port}:{service}"
    if cache_key in _cve_cache:
        return _cve_cache[cache_key]

    cves = []
    # Well-known CVEs mapped to common ports (offline fallback)
    KNOWN_CVES = {
    ...
    if port in KNOWN_CVES:
        cves = KNOWN_CVES[port]

    # Try NVD API if key set
...
def scan_all_ports_cve():
    """Run CVE lookup on all open ports."""
    with lock:
        ports = list(state["open_ports"])
    results = {}
    for p in ports:
        ...
        if cves:
            ...
            # Add threat if critical CVE on open port
            for cve in cves:
                if cve["score"] >= 9.0 and not cve.get("patched"):
                    add_threat(
...
def build_correlation():
    """Link all events by IP — build evidence graph."""
    with lock:
        conns   = list(state["connections"])
    ...
    for c in conns:
        ...
        g["connections"].append({
            "process": c["process"], "raddr": c["raddr"],
            "status": c["status"], "time": ts_now()
        })
        g["total_events"] += 1
        g["last_seen"] = ts_full()
    ...
    for ip, g in graph.items():
        cs = g["correlation_score"]
        ev = g["total_events"]
        ts_val = g["threat_score"]
        final = min(100, cs + (ev * 2) + ts_val)
        g["correlation_score"] = final
        g["risk_level"] = "critical" if final>70 else "high" if final>45 else "medium" if final>20 else "low"
...
# ═══════════════════════════════════════════════
#  MODULE 5 — PDF INCIDENT REPORT GENERATOR
# ═══════════════════════════════════════════════
...
def generate_pdf_report(incident_id):
    ...
    with lock:
        ...
        threats = [t for t in state["threat_log"] if t.get("incident_id")==incident_id
    ...
    fname = REPORTS_DIR / f"{incident_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
    ...
    sev_colors = {"critical":RED,"high":AMBER,"medium":colors.HexColor("#185FA5"),"low":colors.green}

    story = []

    # Header
    story.append(Paragraph("SOC INCIDENT REPORT", h1))
    story.append(Paragraph(f"Classification: <b>CONFIDENTIAL</b>", bod))
    story.append(Spacer(1, 0.3*cm))
    story.append(HRFlowable(width="100%", thickness=2, color=NAVY))
    story.append(Spacer(1, 0.3*cm))

    # Incident summary table
    sev_col = sev_colors.get(inc["severity"], colors.gray)
    summary_data = [
    ...
    t = Table(summary_data, colWidths=[3.5*cm,6*cm,3.5*cm,5*cm])
    ...
    story.append(Paragraph(
        ...
        f"{'Domain <b>'+inc['domain']+'</b> was identified as the associated threat infrastructure. ' if inc.get('domain') else ''}"
    ...
    if inc.get("timeline"):
        ...
        tl_t = Table(tl_data, colWidths=[4*cm, 3*cm, 11*cm])
    ...
    if threats:
        ...
        th_t = Table(th_data, colWidths=[2*cm,2*cm,3.5*cm,5*cm,5.5*cm])
    ...
    if inc.get("evidence"):
        ...
        ev_t = Table(ev_data, colWidths=[4*cm,4*cm,10*cm])
    ...
    story.append(Spacer(1, 0.2*cm))
    story.append(Paragraph(
        f"Report generated: {ts_full()} | System: {sys_info.get('hostname','N/A')} | "
        f"SOC Analyst Level-2 Platform v2.0 | CONFIDENTIAL",
...
def collect_interfaces():
    ...
    for name,addrs in psutil.net_if_addrs().items():
        ...
        info={"name":name,"addresses":[],"is_up":s.isup if s else False,"speed_mbps":s.speed if s else 0}
        for a in addrs:
            fam=str(a.family)
    ...
    if ABUSEIPDB_KEY and not result["is_malicious"]:
        try:
            ...
            if resp.ok:
                ...
                if score>50: result["is_malicious"]=True; result["categories"].append("abuse_reported")
        except: pass
    if domain and not result["is_malicious"]:
        try:
            resp=requests.post("https://urlhaus-api.abuse.ch/v1/host/",data={"host":domain},timeout=4)
            if resp.ok and resp.json().get("query_status")=="is_host":
                result.update({"is_malicious":True,"score":max(result["score"],85),"source":"urlhaus"})
...
def reputation_worker():
    while True:
        ip=None
        with lock:
            if _lq: ip=_lq.popleft()
        if ip:
            with lock: already=ip in _reputation
            if not already:
    ...
    try:
        for c in psutil.net_connections(kind="inet"):
            ...
            if is_ext and rip not in queued:
                queued.add(rip)
                with lock:
                    if rip not in _reputation and rip not in list(_lq): _lq.append(rip)
            rep=_reputation.get(rip)
            flags=[]; mal=False; t_score=0
            if rep:
                t_score=rep.get("score",0)
                if rep.get("is_malicious"): flags.append(f"MALICIOUS (score {t_score})"); mal=True
                elif rep.get("is_restricted"): flags.append("Restricted site")
                elif t_score>50: flags.append(f"Suspicious (score {t_score})")
                if rep.get("categories"): flags+=[cc for cc in rep["categories"] if cc not in flags]
            if rport in SUSPICIOUS_PORTS: flags.append(f"Suspicious port: {SUSPICIOUS_PORTS[rport]}")
            ...
            conns.append({
                ...
                "threat_score":t_score,"is_external":is_ext,
            })
    except: pass
        ...
        for p in psutil.process_iter(["pid","name","username","cpu_percent",
            ...
            try:
                ...
                procs.append({"pid":info["pid"],"name":info["name"] or "unknown",
                    ...
                    "status":info["status"] or "","connections":conns,
                    "suspicious":is_sus,"exe":info["exe"] or ""})
                if is_sus:
...
def detect_hardware():
    ...
    try:
        for proc in psutil.process_iter(["pid","name","exe","username"]):
            try:
                info=proc.info; name=(info["name"] or "").lower(); exe=(info["exe"] or "").lower()
                    ...
                    if any(p in name or p in exe for p in patterns):
                        ...
                        try:
                            for c in proc.connections():
                                if c.raddr and not is_private(c.raddr.ip): ext.append(f"{c.raddr.ip}:{c.raddr.port}")
                        except: pass
                        ...
                        found.append({"pid":info["pid"],"name":info["name"],"hw_type":hw_type,
    ...
    try:
        ...
        if resp.ok:
            ...
            for entry in resp.json().get("urls",[]):
                ...
                if m:
                    d=m.group(1).split(":")[0]; new.add(d); new.add(root_domain(d))
            MALICIOUS_DOMAINS.update(new); _feed_updated=time.time()
...
def api_threats():
    sev=request.args.get("severity","all"); cat=request.args.get("category","all")
    page=max(1,int(request.args.get("page",1))); per=min(100,int(request.args.get("per",50)))
    ...
    return jsonify({"threats":log[start:start+per],"total":total,"page":page,"pages":max(1,(total+per-1)//per)})
...
def api_add_note(inc_id):
    ...
    with lock:
        for inc in state["incidents"]:
            if inc["id"]==inc_id:
                inc["analyst_notes"].append({"time":ts_full(),"analyst":"SOC Analyst","text":data.get("text","")})
                inc["timeline"].append({"time":ts_full(),"stage":inc["stage"],"event":f"Note added: {data.get('text','')[:60]}"})
                break
    ...
    with lock:
        for inc in state["incidents"]:
            if inc["id"]==inc_id:
                ...
                inc["timeline"].append({"time":ts_full(),"stage":inc["stage"],"event":f"Action: {data.get('action','')} — {data.get('detail','')[:60]}"})
        ...
        for inc in state["incidents"]:
            if inc["id"]==inc_id:
                ...
                if cur<3:
                    ...
                    inc["timeline"].append({"time":ts_full(),"stage":inc["stage"],
...
def api_clear():
    with lock:
        state["threat_log"]=[]; state["live_alerts"]=deque(maxlen=300); state["total_alerts"]=0
...
def api_status():
    with lock:
        return jsonify({"status":"running","time":ts_full(),
            "alerts":state["total_alerts"],"incidents":len(state["incidents"]),
            "reportlab":REPORTLAB,"scapy":SCAPY,"abuseipdb":bool(ABUSEIPDB_KEY)})

@app.route("/")
def index(): return "<h3>SOC Level-2 Monitor v2.0 — open dashboard_v2.html</h3>"

# ── ENTRY POINT ──
if __name__=="__main__":
    print("="*62)
    print("  SOC Analyst Level-2 — Threat Detection Platform v2.0")
    print("="*62)
